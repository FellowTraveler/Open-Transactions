...
TEST_MULTI(ot_utility_getNymbox_NoForceDownloadAlreadyHaveNymbox)
{
	EXPECT_CALL_RETURN(NYM_HASH, GetNym_RecentHash(SERVER_ID, NYM_ID));
	EXPECT_CALL_RETURN(NYM_HASH, GetNym_NymboxHash(SERVER_ID, NYM_ID));

	ASSERT_EQ(OT_TRUE, me.ot_utility_getNymbox(SERVER_ID, NYM_ID, false));
}

TEST_MULTI(ot_utility_getNymbox_NoForceDownloadGetNymbox)
{
	int returnValue = REQUEST_NUMBER;

	// force non-matching nym hashes
	EXPECT_CALL_RETURN(NYM_HASH, GetNym_RecentHash(SERVER_ID, NYM_ID));
	EXPECT_CALL_RETURN(NYM_HASH2, GetNym_NymboxHash(SERVER_ID, NYM_ID));
